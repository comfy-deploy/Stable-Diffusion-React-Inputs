//
// This code is comming from the comfyui repository
// https://github.com/comfyanonymous/ComfyUI/blob/master/web/extensions/core/editAttention.js
//
...
export function EditAttention(event: any) {
  ...
  const updatedText = selectedText.replace(
    /\((.*):(\d+(?:\.\d+)?)\)/,
    (match: boolean, text: string, weight: any) => {
  ...
  inputField.dispatchEvent(event2);
}
